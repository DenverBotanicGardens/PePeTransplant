...
pepe.tags <- pepe.all.2023 %>%
  ...
  group_by(TransectID,Y,X,Season,Transect_position) %>%
  summarise(PlantID = cur_group_id())

pepe.all.2023 <- pepe.all.2023 %>%
  select(!"PlantID") %>%
  left_join(pepe.tags, by = c("TransectID","X","Y","Season","Transect_position"))
...
transplant_binom %>%
  ...
  ggplot(  aes(dens, surv)) +
    ...
    , width=100, height=105,units='mm', dpi=300)  
...
### Change to 0 = died, 1 = survived
existing %>%
  inner_join(transplants, by = "TransectID") %>%
  filter(Season == "Spring") %>%
  filter(Date == "20231016") %>%
  mutate(surv = case_when(Height.cm > 0 ~ 1,
                          Height.cm == 0 ~ 0)) %>%
  group_by(TransectID, dens, Block) %>%
  full_join(pepe.transects[pepe.transects$Type_CT_TT == "TT",], by = "TransectID") %>%
  arrange(TransectID) %>%
  replace(is.na(.), 0) %>%
  ggplot( aes(dens, surv)) +
    ...
    ylab("Survival rate of transplants")+
    xlab("Density of existing plants")
...
ciChapman(n1 = recapt$n1[1], n2 = recapt$n2[1], m2 = recapt$m2[1])$Nhat

recapt %>%
  mutate(Nhat = ciChapman(n1,n2,m2)$Nhat,
         ciLWR = ciChapman(n1,n2,m2)$ciNorm)
```
