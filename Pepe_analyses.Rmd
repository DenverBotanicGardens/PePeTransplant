...
## Duplicates? Yes in Transect 16, 11 plants are repeated by plantID
# pepe.all.2023 %>%
#   group_by(Block, TransectID, Date, PlantID) %>%
#   summarize(n=n()) %>%
#   filter(n > 1) %>%
#   group_by(TransectID) %>%
#   summarize(n=n())
# 
# ## Remove duplicates
# pepe.all.2023_nodups <- pepe.all.2023 %>%
#   distinct(TransectID, Block, X,Y,PlantID, .keep_all = TRUE)
# 
# pepe.all.2023_nodups %>%
#   group_by(Block, TransectID, Date, PlantID) %>%
#   summarize(n=n()) %>%
#   filter(n > 1) %>%
#   group_by(TransectID) %>%
#   summarize(n=n())
# 
# repeatIDs <- pepe.all.2023 %>%
#   group_by(Block, TransectID, Date, PlantID, X,Y) %>%
#   summarize(n=n()) %>%
#   filter(n > 1) 
# 
# pepe.all.2023 %>%
#   filter(PlantID %in% repeatIDs$PlantID)
# 
# 
# pepe.all.2023 %>%
#   select(c(TransectID:Height.cm,X:PlantID)) %>%
#   arrange(Block, TransectID, Y,X) %>%
