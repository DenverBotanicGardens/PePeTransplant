...
pepe.all.2023 <- pepe.all.2023 %>%
  ...
  distinct(TransectID, Block, X,Y,PlantID, .keep_all = TRUE)
...
# PlantID unique for each plant in the dataset
# TransectID 10, position 3 repeated; transect 36, position 10
pepe.all.2023 %>%
  filter(!is.na(Transect_position)) %>%
  group_by(TransectID, Block, Transect_position) %>%
  summarise(n = n()) %>%
  filter(n >1)
...
pepe.all.2023 %>%
  ...
  pivot_wider(names_from = Date, values_from = Height.cm, values_fill = 0) %>%
  select(TransectID:PlantID, '20230531', '20231016') %>%
