...
data {
  for(i in 1:N) {
    ...
    # Likelihood
    ### Initial conditions, survival depends on treatment and control
    y_surv[i] ~ dbern(p_surv[i])
    logit(p_surv[i]) <- alpha[ ct[i] ] + bsize * size[i]
    
    y_rep[i] ~ dbern(p_rep[i] * y_surv[i]) ## reproduction dependent on survival
    logit(p_rep[i]) <- alpha[ ct[i] ] + bsize * size[i]
  }
}
...
varcov.alpha[1,2,] <- varcov.alpha[2,1,] <- cor.alpha * sigma.alpha.phi * sigma.alpha.psi
alpha1 <- rmvnorm(N, c(0,0), varcov.alpha)


# parameters are treated as data for the simulation step
data<-list(N=N,ct=ct, alpha=alpha, bsize = bsize, size = size)
