...
## Some double observations in 2024?
# Yes but none are transplants
surv2023_2024 %>% 
  group_by(plantID, recordedBy, date) %>%
  summarise(n = n()) %>%
  filter(n >1) %>%
  left_join(surv2023_2024)%>%
  select(plantID, date, transectID, x, y, isTransplant) %>%
  print(n = 26) 

surv2023_2024 %>%
  filter(plantID %in% c(144,152,258,262,264,272,274,283,287,310,315,320,334)) %>%
  arrange(plantID, date)


surv2023_2024 %>%
  group_by(date, transplantSeason) %>%
  # filter(!duplicated(plantID)) %>%
  filter(transplantSeason != "NULL") %>%
  group_by(date, transplantSeason, plantID) %>%
  summarise(n = n()) %>%
  filter(n > 1) %>%
  group_by(date, transplantSeason) %>%
  summarise(n = n())
...
foo <- surv2023_2024 %>%
  group_by(date, transplantSeason, plantID, recordedBy) %>%
  filter(!duplicated(plantID))

table(foo$date, foo$transplantSeason)
foo %>%
  group_by(date, transplantSeason, plantID) %>%
  summarise(n = n()) %>%
  filter(n > 1)
foo %>%
  filter(plantID == 737) %>%
  arrange(date) %>%
  select(plantID:heightInCentimeters, reproductiveStatus, recordedBy, transplantSeason)
...
surv2023_2024 %>%
  ...
  add_count() %>%
  dplyr::select(plantID:heightInCentimeters, x,y ,recordedBy , transectID,
                transplantSeason:isTransplant, n) %>%
...
## Two plants measured twice in may, more plants measured twice
surv2023_2024 %>%
  group_by(date,transectID, recordedBy, isTransplant, plantID) %>%
  add_count() %>%
  select(id.x:heightInCentimeters, n) %>%
  filter(n > 1) %>%
  arrange(plantID) %>%
  print(n = 35)
...
plants26toadd <- surv2026 %>%
  ...
  select(plant.ID, date, height..cm.:Comments, recordedBy, transectID, x...0.1m., y..0.5m.) %>%
...
pepe23_26 <- surv2023_2024 %>%
  ...
  select(Date,plantID:numberReproductiveStructures,recordedBy, transectID:isTransplant) %>%
  arrange(Date) %>%
...
## Need to groupby plant id and change to "firstObs" and "secondObs"
pepe23_26 %>%
  filter(!is.na(transplantSeason)) %>%
  group_by(Year, transplantSeason, recordedBy) %>%
  summarise(n = n())
...
### have double observations in here. 
## MEDL_Fall goes to 160 percent survival, no!
pepe23_26 %>%
  filter(!is.na(transplantSeason)) %>%
...
## not quite right to average over all observers, should average out or take the max
pepe23_26 %>%
  filter(!is.na(transplantSeason)) %>%
  filter(recordedBy != "MEDL_Fall") %>%
  filter(heightInCentimeters > 0) %>%
...
pepe23_26 %>%
  ...
  filter(recordedBy != "MEDL_Fall") %>%
  group_by(transplantSeason, Year, transectID, block, type_CT_TT, recordedBy) %>%
  summarise(alive = case_when(any(heightInCentimeters > 0) ~ 1,
                              all(is.na(heightInCentimeters)) ~ 0)) %>%
  summarise(perc_surv = n()/5) %>%
...
pepe23_26 %>%
  ...
  summarise(n = n()) %>%
  ggplot(  aes(Year, n, colour = type_CT_TT)) +
