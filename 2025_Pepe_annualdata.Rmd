...
surv_wide_stage <- 
  ...
  dplyr::summarise( # height = mean(heightInCentimeters, na.rm = TRUE),
         # width = mean(basalWidthInCentimeters, na.rm = TRUE),
         ch = ifelse(any(reproductiveStatus > 0) & any(heightInCentimeters > 0), "R", "V"),
         # numReprod = max(numberReproductiveStructures, na.rm = TRUE)
         ) %>%
  ungroup() %>%
  # distinct() %>%
  # mutate(ch = case_when(heightInCentimeters > 0  & reproductiveStatus > 0 ~ "R",
...
pepeMulti <- convert.inp()

multi.proc.ms <- process.data()
