...
surv_wide_robust <-
  ...
  replace_na(list(reproductiveStatus = 0)) %>%
    ## Need to have each year (primary period) have only one strata
  group_by(plantID, Year) %>%
  mutate(Obs = case_when( (heightInCentimeters > 0 & reproductiveStatus == 0) ~ "V",
                          any(heightInCentimeters > 0 & reproductiveStatus == 1) ~ "R",
                          heightInCentimeters == 0 ~ "0")) %>%
    ungroup() %>%
...
cindex <- pepe.dd$p  ## Each session has 1 and 2, no observations session 2 of 2025. time, not Time (that is 0 and 1)
c.missing1 <- cindex %>%
  filter(group %in% c(1,3,5,7,9,11,17,19,23,25,27,29,31,33,35,37,39) & session == "2023" & time == "1")
c.missing3 <- cindex %>%
  filter(group %in% c(16) & session == "2024" & time == "1")
...
Huggins <- mark(pepe.pr, pepe.dd, model = "CRDMSOHug",
                time.intervals = time.intervals, begin.time = 2023)
