...
## Five Fall transplants, one transect, was measured during transplanting and during plot reading. 
## Got rid of the double observations for 5 plots, 2, 4, 6, 8, and 10 above with 
## 
  # "group_by(date) %>%
  # filter(!duplicated(plantID))"
...
## an extra 735, 737, 759, 760, and 762; These are the 5 extras in fall 2023
# surv2023_2024 %>%
#   filter(date == "2023-10-16" & transplantSeason == "Fall") %>%
#   group_by(plantID, transectID, transplantSeason, transectPosition) %>%
#   summarise(n = n()) %>%
#   filter(n > 1)

## Rid of 5 extra in 2023
# surv2023_2024 %>%
#   # filter(recordedBy != "Michelle DePrenger-Levin, Adriana Jacobi") %>%  ## These are the 5 duplicates
#   filter(date == "2023-10-16" & transplantSeason == "Fall") %>%
#   group_by(plantID, transectID, transplantSeason, transectPosition) %>%
#   summarise(n = n()) %>%
#   filter(n > 1)

## One duplicate from Arich and Leah, plantID 1629
# surv2023_2024 %>%
#   filter(date == "2024-06-24" & !is.null(transplantSeason)) %>%
#   group_by(plantID, transectID, transplantSeason, transectPosition, recordedBy) %>%
#   summarise(n = n()) %>%
#   filter(n > 1)
# surv2023_2024 %>%
#   filter(plantID == 1629)
...
## 10 extra records in spring and fall, that's 40 if both have duplicate records, additionally 5 to 10 but that's 50, not 55!!
## Looks like 2, 4, 6, 8, and 10 got double observers and then we stopped doing doubles
## Five transects got double observations in 2024. That means 25 fall and 25 spring were double counted. 
...
#   dplyr::select(Date,Year,plantID:numberReproductiveStructures,transectID:transplantSeason,isTransplant, recordedBy) %>%
...
#    23-Obs1, 23-Obs2    24-Obs1, 24-Obs2  25  26
time.intervals <-  c(0, 1, 0, 1, 1, 1, 0)
### START HERE! need to add a dummy second for 2025 AND 2026
...
## STart above here, make sure fixing variables is correct for 2026
