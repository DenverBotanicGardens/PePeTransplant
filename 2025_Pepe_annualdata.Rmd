...
Phi.table <- get.real(multi.mark.pepe, "S", se = TRUE) %>%
  left_join(TreatTransdf) %>%
  distinct(stratum, group, Trans, estimate, lcl, ucl, V,R, Type) 

Psi.table <- get.real(multi.mark.pepe, "Psi", se = TRUE) %>%  
  left_join(TreatTransdf) %>%
...
dmTypemulti <- model.matrix(~ -1 + type_CT_TT, stage_ch)
dimnames(dmTypemulti)[[2]][length(dimnames(dmTypemulti)[[2]])]

stage_ch %>%
  dplyr::select(ch) %>% # only keep the capture history
  bind_cols(dmTypemulti) %>%   # bind the design matrix based on Type
  mutate(type_CT_TTTT = paste(type_CT_TTTT, ";", sep= "")) %>%   # add a semicolon to every row of type TT
...
multi.proc.type <- process.data(stage_ch, begin.time = 2023, model = "Multistrata", groups = "type_CT_TT")
...
multi.mark.pepeType <- mark(multi.proc.type, multi.ddl.type, begin.time = 2023, model = "Multistrata",
                        model.parameters = list(S = list(formula = ~ 1 + type_CT_TT),
                                                p = list(formula = ~ 1),
                                                Psi = list(formula = ~ -1 + stratum:tostratum:type_CT_TT))) 

Phi.table.type <- get.real(multi.mark.pepeType, "S", se = TRUE) %>%
  distinct(stratum, group, type_CT_TT, estimate, lcl, ucl, V, R)  # , .keep_all = TRUE

Psi.table.type <- get.real(multi.mark.pepeType, "Psi", se = TRUE) %>%
  distinct(stratum, tostratum, R, V, toR, group, type_CT_TT, estimate, lcl, ucl) 
...
```{r}
Phi.table.type %>%
  ggplot(  aes(type_CT_TT,estimate,  color = stratum)) +
    geom_point(position = position_dodge(width = .75)) +
    geom_errorbar(aes(ymin = lcl, ymax = ucl),position = position_dodge(width = .75))

Psi.table.type %>%
  ggplot(  aes(type_CT_TT, estimate, color = tostratum)) +
    geom_point(position = position_dodge(width = .75)) +
    geom_errorbar(aes(ymin = lcl, ymax = ucl),position = position_dodge(width = .75))
```
...
MPM.pepe <- matrix( c(Phi.table$estimate[Phi.table$stratum == "V"] * (1-Psi.table$Psi[Psi.table$stratum == "V"]),
