...
CountStage <- Appendix %>%
  ...
  select(transectID, type_CT_TT,plantID, InitreproStatus)
  # mutate(reproductiveStatus = as.factor(reproductiveStatus)) %>%
  # ungroup() %>%
  # group_by(Year, transectID, transplantSeason, reproductiveStatus) %>%
  # summarise(countStage = n()) %>%
  # rename(InitReproStatus = reproductiveStatus)

CountStage %>% filter(Year == 2023)
...
surv_wide %>% head()
  filter(isTransplant == 1) %>%
  left_join(CountStage[,-1]) %>%
  group_by(Year, transectID, type_CT_TT, plantID, x, y, transplantSeason, transectPosition, InitreproStatus) %>%
  summarise(heightInCentimeters = max(heightInCentimeters, na.rm = TRUE),
            basalWidthInCentimeters = max(basalWidthInCentimeters, na.rm = TRUE),
            reproductiveStatus = max(reproductiveStatus, na.rm = TRUE),
            numberReproductiveStructures = max(numberReproductiveStructures, na.rm = TRUE)) %>%
  replace_na(list(heightInCentimeters = 0))  %>%
  ungroup() %>%
  group_by(Year, transectID, type_CT_TT,transplantSeason, InitreproStatus) %>%
  summarise(SurvPerc = case_when(n() == 0 ~ NA,
                                 n() > 0 ~ length(heightInCentimeters > 0)/n())) %>% 
  filter(!is.na(SurvPerc)) %>%
  ungroup() %>% 
  filter(Year != 2023) %>%
ggplot(  aes(transplantSeason, SurvPerc, fill = Year)) +
  geom_boxplot(position = position_dodge(width = 1)) +
  geom_point(position = position_jitterdodge(dodge.width = 1))+
  theme_bw() +
  xlab("Transplant Season")+
  ylab("Percent Survival")+
  scale_fill_manual(values = c("tomato","grey")) +
  facet_grid(Year~InitreproStatus)
```
