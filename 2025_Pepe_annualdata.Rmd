...
survey2bind %>%
  group_by(recordedBy, Date, transectID) %>%
  summarise(n = n()) %>%
...
survey2bind %>% 
  # arrange(transectID, surveyID) %>%
  group_by(Date, transectID, Year) %>%
  summarise(n = n()) %>%
  print(n = 40)
...
surv_wide_robust <-
  ...
  group_by(plantID, Year) %>%
    mutate(Obs = if_else(mapply(grepl,"R", Obs), gsub("V","R",Obs), Obs)) %>%
...
## surv_wide_robust up until pivot_wider: surveyID not applied appropriately
surv_wide_robust %>%
  group_by(plantID, transectID, isTransplant,transplantSeason, type_CT_TT, surveyID) %>%
  add_count() %>%
  select(Date, plantID, surveyID, recordedBy, Obs, Year, n) %>%
  filter(n > 1) %>%
  arrange(plantID, surveyID)
## Up to left_join survey2bind
surv_wide_robust %>%
  group_by(plantID, transectID, surveyID) %>%
  add_count() %>%
  select(Date, plantID, surveyID, recordedBy, n) %>%
  filter(n > 1) %>%
  arrange(plantID, Date,surveyID)
