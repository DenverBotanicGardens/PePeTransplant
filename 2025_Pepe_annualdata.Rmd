...
plants2025 <- plants2025 %>%
  ...
  rename(reproductiveStatus = Reproductive.Status,
         numberReproductiveStructures = Number.Reproductive.Structures)
...
surv_wide <- 
  ...
  #                         is.na(heightInCentimeters) ~ "0")) %>%
    # dplyr::select(plantID:numberReproductiveStructures,transectID:isTransplant) %>% 
  ungroup() %>%
    distinct() %>%
...
surv_wide %>%
  select(plantID, transectID, `2023-10-16`:`6/24/2025`) %>%
  arrange(transectID, plantID)
...
############# START HERE
Working other than no surveys in 2025! What!?!
...
surv_wide_robust <-
  ...
  left_join(data.frame(survey2bind[,c(1:5)]), by = c("transectID" = "transectID", "recordedBy" = "recordedBy", 
                                                     "Date" = "Date")) %>%
    ## Why are there only measures in survey 2??
...
## Control only in 2023-10-16, none in 2023-05-XX
...
## Should be four for each of them, but some we didn't get double counts in 2024
surv_wide_robust %>%
  group_by(transectID,survey) %>%
  summarise(n = n()) %>%
  group_by(transectID) %>%
  summarise(n = n()) %>%
  print(n = 38)
## Only keep the combinations of surveys for each transect and number them either 1:4 or 1,2,4
surv_wide_robust %>%
  group_by(transectID,survey) %>%
  summarise(plants = n()) %>%
  group_by(transectID) %>%
  add_count() %>%
  # mutate(surveyNum = case_when(n == 3 ~ c(1,2,4),
  #                              n == 4 ~ 1:4))
  mutate(survey = as.numeric(as.factor(survey))) %>%
  mutate(survey = case_when((n == 3 & survey == 3) ~ 4,
                            TRUE ~ survey)) %>%
  print(n = 100)

## Any that are always dead like AsMi??
surv_wide <- surv_wide %>%
