...
## What is recorded as 118 is in slot 8 (but notes should say "-> origin" instead of "center") and have the measurements for slot 8 but 
data2024 %>%
  filter(plantID >= 110 & plantID < 119 & date %in% c("2023-10-16" , "2023-05-31"))

plants2024 %>%
  filter(plantID == 118) ## However, this is transect 8, slot 10 and fall
plants2024 %>%
  filter(transectID == 8 & !is.na(Transect_position)) ## just missing slot 8 so add it
row2add <- c(id = max(plants2024$id)+1, 
             transectID = 8,
             Block = 4,
             x = 0.75,
             y = 3.75,
             plantID = max(plants2024))
...
## This is a spring transplant! Not Fall
surv2023_2024 %>%
  filter(Season == "Fall" & date == "2023-05-31")

surv2023_2024 %>%
  filter(plantID == 118) %>%
  arrange(Transect_position,date, Season) %>%
  select(c(plantID:date, transectID,Transect_position))

surv2023_2024 %>%
  filter(transectID == 10 & Season %in% c("Spring","Fall") & date == "2023-10-16")

## Any other duplicate data plantIDs??
dups <- surv2023_2024 %>%
  filter(date %in% c("2024-06-24", "2024-06-25"))


names(table(dups$plantID)[table(dups$plantID) > 2]) ## 118 is the only one with 6, maybe only duplicated?
