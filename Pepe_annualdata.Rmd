...
load("C:/Users/deprengm/Denver Botanic Gardens/Conservation - General/AllProjectsBySpecies/Penstemon-penlandii/Penstemon-penlandii_Projects/Tri-State_Transplant_2023-2026/Data/2025_NewPlants.Rdata")
...
# surv_wide <- 
  ...
  #                           date == "6/24/2025" ~ "d25", 
  #                           date == "6/25/2025" ~ "d25")) %>%
  pivot_wider(names_from = date,# names_from = c(Survey, recordedBy),
              values_from = ch,
...
## Why are there two still in 2024? 
  
## Need to remove the transects that recordedBy didn't view, change to observation 1 and 2 for each plot
plants25toadd %>%
  group_by(recordedBy, transectID) %>%
  summarise(n = n())
...
# Add 2025 
```{r}
# add rows, then pivot wider
add2025 <- plants2025_updated %>% 
  left_join(trans2024, by = c("transectID" = "transectId")) %>%
  filter(duplicateNew == 0) %>%
  mutate(year = format(as.Date(date, format = "%m/%d/%Y"),"%Y")) %>%
  mutate(surv = case_when(heightInCentimeters > 0 ~ 1,
                          heightInCentimeters == 0 ~ 0)) %>%
  select(c(year, transectID, type_CT_TT, plantID, heightInCentimeters, surv)) 
...
exist2023_2024 %>%
  ...
  pivot_wider(id_cols = c(transectID, type_CT_TT, plantID), names_from = year, names_prefix = "Year",
              values_from = surv) %>%
  group_by(transectID, type_CT_TT) %>%
  summarise(survRate24 = sum(Year2024[Year2023 == 1], na.rm = TRUE)/sum(Year2023, na.rm = TRUE),
            survRate25 = sum(Year2025[Year2023 == 1], na.rm = TRUE)/sum(Year2023, na.rm = TRUE),
            recruit = length(is.na(Year2023))) %>%
  filter(!is.infinite(survRate25)) %>%
  filter(!is.na(survRate25)) %>%
ggplot(   aes(type_CT_TT, survRate25)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(height = 0, pch = 16)+
  theme_bw() +
  scale_x_discrete(labels = c("CT" = "Control", "TT" = "Transplant")) +
...
lm1 <- glm(surv ~ type_CT_TT + year, family = binomial(link = "logit"),data = existGLM[existGLM$year != 2023,])
summary(lm1)
```
